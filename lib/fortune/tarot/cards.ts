 ...
 * 카드 ID를 이미지 파일명으로 변환
 * 파일명 패턴 (모두 소문자):
 * - 메이저 아르카나: the-fool, the-magician, judgement 등
 * - 에이스: ace-of-wands, ace-of-cups 등
 * - 숫자 카드 2-10: wands-ii, cups-x 등
 * - 코트 카드: page-of-wands, knight-of-cups, queen-of-swords, king-of-pentacles 등
 */
export function getCardImageFilename(card: TarotCardInfo): string {
  ...
  if (numericRanks.includes(rank)) {
    ...
    return `${suit}-${roman}`; // wands-ii, cups-x
  }

  // 코트 카드 (page, knight, queen, king)
  return `${rank}-of-${suit}`; // page-of-wands, knight-of-cups
}
